    ...
    while len(queue) > 0:
        ...
        for v,u in graph.edges:
            if u == minelement and v in queue:
                ndist = dist[minelement] + minelement.round_distance(v)*2 # *2; conversion from distance to fuel req
                if ndist < dist[v]:
                    dist[v] = ndist
...
id = "two-cities-sequential-%dnodes-%dsize-%ddegree-%dmindistance-%dtrucks-%dpackages-%dseed" % (nodes, size, degree, epsilon, trucks, packages, seed)
...
for v in city_a.vertices:
    for u in city_b.vertices:
        if v.distance(euclidean_graph.Point(u.name, u.x + 2*size, u.y)) < min_connect_distance:
            min_tuple = (v, u)
            min_connect_distance = v.distance(euclidean_graph.Point(u.name, u.x + 2*size, u.y))

print("  ; %d,%d <-> %d,%d" % (min_tuple[0].x, min_tuple[0].y, min_tuple[1].x + 2*size, min_tuple[1].y))

symmetric_road("city-1-"+min_tuple[0].name, "city-2-"+min_tuple[1].name, min_connect_distance)
