...
def shortest_route(ca, cb, ox, oy):
    ...
    for v in city_a.vertices:
        for u in city_b.vertices:
            if v.distance(euclidean_graph.Point(u.name, u.x + ox, u.y + oy)) < min_connect_distance or min_connect_distance == -1.0:
                min_tuple = (v, u)
                min_connect_distance = v.distance(euclidean_graph.Point(u.name, u.x + 2*size, u.y))

    return min_tuple, min_connect_distance


if len(sys.argv) != 8:
    raise SystemExit("Usage: <nodes> <size^(1/2)> <degree> <mindistance> <nr-trucks> <nr-packages> <seed>")
...
city_a = euclidean_graph.generate_connected(nodes, size, size, connect_distance, epsilon)
city_b = euclidean_graph.generate_connected(nodes, size, size, connect_distance, epsilon)
city_c = euclidean_graph.generate_connected(nodes, size, size, connect_distance, epsilon)


id = "three-cities-sequential-%dnodes-%dsize-%ddegree-%dmindistance-%dtrucks-%dpackages-%dseed" % (nodes, size, degree, epsilon, trucks, packages, seed)
...
for u, v in city_c.edges:
    print("  ; %d,%d -> %d,%d" % (u.x + size,u.y + 2*size, v.x + size, v.y + 2*size))
    dist = u.round_distance(v)
    road("city-3-"+u.name, "city-3-"+v.name, dist)

connect_ab, dist_ab = shortest_route(city_a, city_b, 2*size, 0)
print("  ; %d,%d <-> %d,%d" % (connect_ab[0].x, connect_ab[0].y, connect_ab[1].x + 2*size, connect_ab[1].y))
...
package_loc = { }
for i in range(packages):
    package_loc["package-%d" % (i+1)] = (random.randint(1,3), random.choice(city_a.vertices).name)
    print("  (at package-%d city-%d-%s)" % ( i+1, package_loc["package-%d" % (i+1)][0], package_loc["package-%d" % (i+1)][1]))

for i in range(trucks):
    print("  (at truck-%d city-%d-%s)" % (i+1, random.randint(1,3), random.choice(city_a.vertices).name))
...
for i in range(packages):
    ...
    while rc == package_loc["package-%d" % (i+1)][0] and rl == package_loc["package-%d" % (i+1)][1]:
        rc = random.randint(1,3)
        rl = random.choice(city_b.vertices).name
