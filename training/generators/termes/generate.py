...
def do_it(init, goal, output_type, reduce_height_init=0, reduce_height_goal=0, grid_slack=0, multi_robot=False, ensure_plan=False, store_plan=False, dont_remove_slack=False):
    i_grid =  [list(map(lambda x : max(0, int(x) - reduce_height_init),l.strip().split())) for l in init.readlines() ]
    g_grid =  [list(map(lambda x : max(0, int(x) - reduce_height_goal),l.strip().split())) for l in goal.readlines() ]

    if not dont_remove_slack:
        i_grid = remove_slack (i_grid)
        g_grid = remove_slack (g_grid)

    grid_size_x = max(len(i_grid[0]), len(g_grid[0])) + grid_slack
    grid_size_y = max(len(i_grid), len(g_grid)) + grid_slack
    ...
    instance_name_list += ["{:04d}".format(problem_size), "{}x{}x{}".format(grid_size_x, grid_size_y, max_height)]

    if not init.name.split('/')[-1].startswith("empty"):
        instance_name_list += [init.name.split('/')[-1].replace('.txt', '')]

    instance_name_list += [goal.name.split('/')[-1].replace('.txt', '')]

    instance_name = "-".join(instance_name_list)



    if output_type.startswith("pddl"):
        lines = generate_for_pddl(instance_name, grid_size_x, grid_size_y, max_height, robots, depots, init_grid, goal_grid, multi_robot)
        if output_type == 'pddl':
            print ("\n".join(lines))
        else:
            with open(os.path.join(here, "instances", "p-{}.pddl".format(instance_name)), 'w') as f:
                f.write ("\n".join(lines))
    elif output_type == "readable":
    ...
    if store_plan:
        if not plan:
            plan = solve(grid_size_x, grid_size_y, max_height, robots, depots, init_grid, goal_grid)
        if plan:
            with open(os.path.join(here, "plans", "p-{}.pddl".format(instance_name)), 'w') as f:
                f.write ("\n".join(plan))
