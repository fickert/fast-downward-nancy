...
void AStarLookaheadSearch::initialize(const GlobalState &initial_state) {
	...
	auto eval_context = EvaluationContext(initial_state, 0, false, nullptr);
	open_list->insert(eval_context, initial_state.get_id());
}

auto AStarLookaheadSearch::search() -> SearchStatus {
	while (expanded < lookahead_bound) {
		if (open_list->empty())
			return FAILED;
		...
		auto eval_context = EvaluationContext(state, node.get_g(), false, nullptr);
		...
		for (auto op_id : applicable_ops) {
			...
			if (succ_node.is_new()) {
				auto succ_eval_context = EvaluationContext(succ_state, node.get_g() + op.get_cost(), false, nullptr);
				...
				auto succ_eval_context = EvaluationContext(succ_state, succ_node.get_g(), false, nullptr);
				open_list->insert(succ_eval_context, succ_state.get_id());
			}
		}
	}
