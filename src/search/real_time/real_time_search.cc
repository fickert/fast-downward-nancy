...
RealTimeSearch::RealTimeSearch(const options::Options &opts)
	...
	if (opts.get<bool>("learning")) {
		auto learning_evaluator = std::make_shared<LearningEvaluator>(heuristic);
		dijkstra_learning = std::make_unique<DijkstraLearning>(learning_evaluator, state_registry);
		heuristic = learning_evaluator;
	}

	switch (LookaheadSearchMethod(opts.get_enum("lookahead_search"))) {
	case LookaheadSearchMethod::A_STAR:
		lookahead_search = std::make_unique<AStarLookaheadSearch>(state_registry, opts.get<int>("lookahead_bound"), heuristic, static_cast<bool>(dijkstra_learning));
		break;
	default:
...
SearchStatus RealTimeSearch::step() {
	...
	if (dijkstra_learning)
		dijkstra_learning->apply_updates(lookahead_search->get_predecessors(), lookahead_search->get_frontier(), lookahead_search->get_closed());
